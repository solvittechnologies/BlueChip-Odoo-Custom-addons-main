...
class ProjectUpdate(models.Model):
    ...
    @api.multi
    @api.depends('amount_total', 'disbursement_retirement_amount_ids')
    def get_actual_implementation_cost(self):
        for rec in self:
            retirement_total = sum(rec.disbursement_retirement_amount_ids.mapped('fund_amount'))
            rec.actual_implementation_cost = rec.amount_total - retirement_total
